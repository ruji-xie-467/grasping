...
def main():
    ...
    while not rospy.is_shutdown():
        i_back += 1
        try:
            goal_init = PoseStamped()
            goal_init.header.frame_id = "base"

            #x, y, and z position
            # [0.551, 0.690, -0.049]
            goal_init.pose.position.x = 0.551
            goal_init.pose.position.y = 0.690
            goal_init.pose.position.z = -0.049

            #Orientation as a quaternion
            goal_init.pose.orientation = Quaternion(q[0],q[1], q[2], q[3])


            plan = planner.plan_to_pose(goal_init, list())

            # raw_input("Press <Enter> to LEAVE pose: ")
            print("Back pose %d" % i_leave)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        while not rospy.get_param('broad_grasp_base'):
            rospy.sleep(0.5)
        count_down_clock(3)  ## wait 3 sec to set_param()
        rospy.set_param('broad_grasp_camera', 0)
        rospy.set_param('broad_grasp_base', 0)
        main()
        rospy.set_param('executed', 1)
