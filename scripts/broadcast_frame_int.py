...
"""
rospy.init_node('broadcast_grasp_base', anonymous=True)


tf_listener = tf.TransformListener(rospy.Duration(1))
while not rospy.is_shutdown:

    while not rospy.get_param('broad_grasp_camera'):
        rospy.sleep(0.5)
    ...
    rospy.set_param('get_grasp_camera', 1)
    rospy.set_param('broad_grasp_base', 1)
    rospy.set_param('good_grasps', len(qs))
    count_down_clock(3)

    while not rospy.is_shutdown():
        if rospy.get_param('executed')
            rospy.set_param('executed', 0)
            break
